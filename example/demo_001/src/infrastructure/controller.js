...
const getRequestWithContext = (req) => {
    return req.context;
};
export function generateController(router, useCases) {
    //
    useCases.forEach(({ funcInstance, funcMetadata }) => {
        //
        const useCase = funcInstance;
        const data = funcMetadata.additionalDecorators.find((x) => x.name === "Controller")?.data;
        if (!data)
            throw new Error("undefined path and method");
        router[data.method](data.path, async (req, res) => {
            //
...
export const handleJWTAuth = (secretKey, fieldName) => async (req, res, next) => {
    //
    try {
        //
...
export function printController(usecases) {
